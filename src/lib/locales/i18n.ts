...
function translate(locale, key, vars) {
	...
	if (!key) throw new console.log('no key provided to $t()');
	if (!locale) throw new console.log(`no translation for key "${key}"`);

	// Grab the translation from the translations object.
	let text = translations[locale][key];

	if (!text) throw new console.log(`no translation found for ${locale}.${key}`);
